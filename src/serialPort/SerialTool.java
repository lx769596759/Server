...
public class SerialTool {
	...
	public static void stopMeasure() {
		// 关闭接收线程
		RecieveFromPort.isRun = false;
		RecieveFromCom.isRun=false;
		try {
			// 发送停止扫描命令
			SerialTool.sendToPort(RecieveFromPort.serialCom[0], new byte[] {
					(byte) 0xA5, 0x25 });
			Thread.sleep(100);
			// 发送关闭电机转动命令
			SerialTool.sendToPort(RecieveFromPort.serialCom[0], new byte[] {
					(byte) 0xA5, (byte) 0xF0, 0x02, 0x00, 0x00, 0x57 });

			// 清空串口的数据
			for (int j = 0; j < RecieveFromPort.serialCom.length; j++) {
				byte[] b1 = new byte[10240];
				int temp = 0;
				while ((temp = RecieveFromPort.serialCom[j].getInputStream()
						.read(b1)) != 0) {
			...
			ServiceClient.logger.error("关闭雷达数据接收失败",e);
		}
	}
}
